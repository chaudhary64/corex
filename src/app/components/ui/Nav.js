...
const Nav = () => {
  ...
  useGSAP(() => {
    ...
    mm.add("(width < 64rem)", (context) => {
      ...
      let splitedMobileSpans = SplitText.create(mobileSpans, {
        type: "lines, chars",
        mask: "lines",
      });
      let splitedMobileDesc = SplitText.create(mobileDescRef.current, {
        type: "lines, chars",
  ...
  return (
    <header className="w-[90%] max-w-[1440px] mx-auto relative">
      <nav className="py-4 flex items-center justify-between border-b border-gray-500">
        ...
        <button ref={hamburgerRef} className="cursor-pointer lg:hidden">
          <svg
